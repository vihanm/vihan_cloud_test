...
data "template_file" "certificates" {
    template = "${file("template/kubernetes-csr.json")}"
...
/*resource "null_resource" "certificates" {
  ...
  provisioner "local-exec" {
    command = "cd ../cert; cfssl gencert -initca ca-csr.json | /usr/local/bin/cfssljson -bare ca; cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -profile=kubernetes ../terraform/template/kubernetes-csr.json | /usr/local/bin/cfssljson -bare kubernetes"
  }
}*/